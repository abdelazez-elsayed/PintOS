...
void get_args (char *file_name, char *args[], int *argc);
void push_args_to_stack(char* args[], int argc , void** esp);

void
get_args (char *file_name, char *args[], int *argc)
{
    ...
    char tmp[len+1];
    memcpy (tmp, file_name, len+1);
    for (token = strtok_r (tmp, " \t", &save_ptr); token != NULL;
         token = strtok_r (NULL, " \t", &save_ptr))
         {
            if(arg_len==50)thread_exit();
           int len = (int) strlen(token) + 1;
           args[arg_len] = (char *) malloc(sizeof(char) * (len+1));
           strlcpy(args[arg_len], token, len);
           args[arg_len][len-1] = '\0';
           arg_len++;
         }
    *argc = arg_len;
    free (token);
...
void push_args_to_stack(char* args[], int argc , void** esp){
  ...
  for (int i = argc - 1; i >= 0; i--)
  {
    ...
    *esp_c -= len+1;
    args_addresses[i] = *esp_c;
    memcpy (*esp_c, args[i], len);
    padding_count += len;
  }

  // Push Padding
  int remain = (4 - (padding_count % 4));
	*esp_c -= remain;
  ...
  *esp_c -= sizeof(temp);
  ...
  for (int i = argc - 1; i >= 0; i--)
  {
    *esp_c -= sizeof(int); // minus 4 bytes - move integer
    memcpy (*esp_c, &args_addresses[i], sizeof(char *));
  }

  // Argv
  temp = *esp_c;
  *esp_c -= sizeof(temp);
  memcpy (*esp_c, &temp, sizeof(temp));

  // Push argc
  *esp_c -= sizeof(int);
  memcpy (*esp_c, &argc, sizeof(int));
...
{
  char *fn_copy=NULL,*argg,*program=NULL;
  struct tcb *tcb=NULL;
  tid_t tid;
  ...
  program = strtok_r(program, " ",&argg); //get executable file name
  /* Initialize TCB*/
  ...
  //wait until load and pushing argv to stack
  sema_down(&tcb->sema);
  if(tcb->tid == tid)
  {
...
start_process (void *ptr)
{
  ...
  if (success) // push argv to stack
  {
    ...
    push_args_to_stack(args,argc,&if_.esp);
    free_args(args,argc);
  }
